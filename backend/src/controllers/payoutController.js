    ...
    const result = await transaction(async (client) => {
      ...
      const payoutResult = await client.query(
        `INSERT INTO payouts (circle_id, recipient_id, amount, cycle_number, status)
         ...
         RETURNING *`,
        [circle_id, recipient_id || circle.next_payout_user_id, payoutAmount, circle.current_cycle]
      );
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        is_ready_for_payout: pool.members_paid >= pool.total_active_members || poolProgress >= 80
      }
    });
  } catch (error) {
    next(error);
